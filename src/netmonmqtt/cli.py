...
def get_check_entities(check_type: str, netmon: NetMon, name: str, expire: int, via_device: Optional[MQTTDevice] = None):
    cleaned_name = name.lower().replace(" ", "_")
    if check_type == "dns":
        return (
            ConnectivityEntity(netmon, f"{name} DNS Resolution", f"{cleaned_name}_dns_resolution", expire=expire, via_device=via_device),
            LatencyEntity(netmon, f"{name} DNS Lookup Latency", f"{cleaned_name}_dns_latency", expire=expire, via_device=via_device),
        )
    if check_type == "ping":
        return (
            ConnectivityEntity(netmon, f"{name} Reachability", f"{cleaned_name}_reachability", expire=expire, via_device=via_device),
            LatencyEntity(netmon, f"{name} Latency", f"{cleaned_name}_latency", expire=expire, via_device=via_device),
        )
    ...
    for tunnel_config in config.tunnels:
        ...
        if tunnel_config.ping_check is not None:
            tunnel.independant_checks.add(Check(
                ...
                get_check_entities(
                    tunnel_config.ping_check.check_type,
                    tunnel,
                    tunnel_config.ping_check.name,
                    tunnel_config.ping_check.expire,
                    via_device=netmon,
                ),
        ...
        if tunnel_config.dns_check is not None:
            tunnel.independant_checks.add(Check(
                ...
                get_check_entities(
                    tunnel_config.dns_check.check_type,
                    tunnel,
                    tunnel_config.dns_check.name,
                    tunnel_config.dns_check.expire,
                    via_device=netmon,
                ),
